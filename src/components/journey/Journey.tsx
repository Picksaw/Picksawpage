...
import { CorridorScene, focusedIndex, focusedSection, stations, SECTION_COUNT } from "./Corridor";
import PreviewModal from "../PreviewModal";
import MagneticButton from "../ui/MagneticButton";
import { useSound } from "../../audio/SoundProvider";
import { getLenis } from "../../lib/lenis";
import { getStorm, stormIntensity } from "../../lib/stormStore";
...
export default function Journey({ lang }: { lang: Lang }) {
  ...
  /** which end-of-hall panel is framed, -1 when none */
  const [sectionIdx, setSectionIdx] = useState(-1);
  /**
   * Weather, read from the EXISTING storm store rather than a new one,
   * so the 2D canvas and the corridor share one storm.
   */
  const weather = useRef({ storm: 0.35, bolt: 0 });
  const [, forceWeather] = useState(0);
  ...
  useMotionValueEvent(scrollYProgress, "change", (v) => {
    ...
    const sec = focusedSection(v);
    setSectionIdx((prev) => (prev === sec ? prev : sec));
    // the walk ends → hand the page back to normal sections
    // (early enough that no painting can catch clicks near the end)
    // The hall runs all the way to the contact panel now, so the canvas
    // may only stand down once the exit station is genuinely reached.
    const endFaded = v > 0.995;
  ...
  // sampled a few times a second — this does not need per-frame React
  useEffect(() => {
    const tick = window.setInterval(() => {
      weather.current.storm = 0.25 + stormIntensity() * 0.6;
      weather.current.bolt = getStorm().bolt;
      forceWeather((n) => (n + 1) % 1000);
    }, 120);
    const off = onLightning((i) => {
      weather.current.bolt = Math.max(weather.current.bolt, i);
      forceWeather((n) => (n + 1) % 1000);
    });
    return () => {
  ...
  /** Contact links live in the world now; open them like any anchor. */
  ...
  return (
    <>
      ...
      <div
        ...
        aria-hidden
        style={{
          // P + headline + one screen per painting + one per section + exit
          height: `${100 + 100 + TEMPLATES.length * 92 + SECTION_COUNT * 92 + 55}vh`,
        }}
      />
      ...
      >
        ...
        >
          {/* The layered CityFog supplies the near/mid depth now, so the
              hard scene fog only needs to close off the far end. */}
          <fog attach="fog" args={["#06080f", 10, 52]} />

          {/* The facades are lit PBR now, so the scene needs a real key:
              cool moonlight from above-left plus a dim sky fill. */}
          <hemisphereLight args={["#2b4260", "#05070d", 0.5]} />
          <ambientLight intensity={0.16} color="#22304a" />
          <directionalLight position={[-6, 9, 5]} intensity={1.5} color="#cfe0ff" />
      ...
      <AnimatePresence>
        {focusedItem && sectionIdx < 0 && !faded && !selected && (
          <motion.div
            key="focus-bar"
